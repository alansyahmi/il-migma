    ...
    const staleRows = await client.execute({
        sql: `
            SELECT
                ...
                pm.gender,
                pm.form_fem_pattern,
                pm.form_masc_pattern,
                pm.form_plural_pattern
            FROM participle_morphology pm
            LEFT JOIN entries e ON e.id = pm.entry_id
            WHERE COALESCE(LOWER(TRIM(e.pos)), '') <> ?
            ORDER BY pm.entry_id
        `,
        args: ['participle'],
    });
