...
function InflectionCell({ row }: { row: InflectionRow }) {
    ...
    const hidden = shouldHideSurface(row, hideTheoreticalForms);
    const markerEl = !hideTheoreticalForms && row.marker === 'theoretical' ? (
        <span className="text-black/45">*</span>
    ) : !hideTheoreticalForms && row.marker === 'auto_generated' ? (
        <span className="text-black/45">✦</span>
    ) : null;
    const form = hideTheoreticalForms ? stripTheoreticalPrefix(row.form) : row.form.trim();
...
function EntryCard({ result, index }: { result: SearchResult; index: number }) {
    ...
    const visibleInflections = result.inflections.filter(row => !shouldHideSurface(row, hideTheoreticalForms));
