...
function buildHybridZokkConjugation(entry: any = {}, forms: ReturnType<typeof generateZokkForms>): VerbConjugationTable | null {
    ...
    const imperfectCitationSource = useSFormIIHybrid
        ? `ji${lemma}`
        : normalizeTextField(hybrid?.form_ii_imperfect || '');
    const startsWithVowel = /^[aeiouàèìòùâêîôû]/i.test(base);
    const imperfectCitation = startsWithVowel
        ? imperfectCitationSource.replace(/^ji/, 'j')
        : imperfectCitationSource;
    const imperfectStem = imperfectCitationSource.replace(/^(?:ni|ti|ji|n|t|j)/, '') || `${usesWeakHybridEndings ? `i${base}a` : `${base}a`}`;
    ...
    const prefixes = startsWithVowel
        ? ['n', 't', 'j', 't', 'n', 't', 'j']
...
function buildSinglePreviewCell(
...
function buildPreviewCell(
    ...
    const items = parseMultiFormString(value);
    if (items.length <= 1) {
        const normalizedValue = items[0] || normalizeTextField(value);
        return buildSinglePreviewCell(normalizedValue, type, form, entry, attested, fallback, storedFallback);
    }

    const cells = items.map(item => buildSinglePreviewCell(item, type, form, entry, attested, fallback, storedFallback));
    const firstWithId = cells.find(c => c.entryId);
    const hasAutoGenerated = cells.some(c => c.marker === 'auto_generated');
    return {
        value: cells.map(c => c.value).join('; '),
        marker: hasAutoGenerated ? 'auto_generated' : 'plain',
        entryId: firstWithId?.entryId,
    };
}
...
export function parseMultiFormString(value: unknown): string[] {
    if (!value) return [];
    if (Array.isArray(value)) {
        return value.flatMap(item => parseMultiFormString(item));
    }
...
export function formatMultiFormString(value: unknown): string {
    const parsed = parseMultiFormString(value);
    return parsed.join('; ');
}
