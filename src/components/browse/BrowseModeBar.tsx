...
const FACETS: Array<{ key: BrowseFacetKey; to: string; labelKey: string }> = [
    { key: 'entries', to: '/browse', labelKey: 'entries' },
    { key: 'pattern', to: '/browse/pattern', labelKey: 'pattern' },
    { key: 'source', to: '/browse/source', labelKey: 'source' },
    { key: 'suffix', to: '/browse/suffix', labelKey: 'suffix' },
];
    ...
    return (
        <div ref={rootRef} className="relative inline-flex shrink-0">
            <button
                ...
                aria-label={`${term('browse-mode')}: ${term(activeFacet.labelKey)}`}
                onClick={() => setOpen((value) => !value)}
                className={cn(
                    'inline-flex items-center gap-1.5 bg-transparent p-0 font-[inherit] text-[inherit] leading-[inherit] tracking-[inherit] text-current transition-colors',
            ...
            >
                <span className="whitespace-nowrap">{term(activeFacet.labelKey)}</span>
                <ChevronDown
                    size={14}
                    className={cn('shrink-0 translate-y-[1px] text-black/65 transition-transform duration-200', open && 'rotate-180')}
                />
            </button>
